...
class ACGAN():
    ...
    def __init__(self, data_path, latent_dim, batch_size=32):
    ...
    def train(self, epochs, save_every):
        ...
        for epoch in range(epochs):
            ...
            for index in range(nb_batches):
                ...
                epoch_disc_loss_real.append(self.discriminator.train_on_batch(image_batch, [np.array([1] * bs), label_batch]))

                generated_images, sampled_labels = self.generate_fake(bs)

                epoch_disc_loss_fake.append(self.discriminator.train_on_batch(generated_images, [np.array([0] * bs), sampled_labels]))
            ...
            if epoch % 1 == 0:
                ...
                discriminator_train_loss_real = np.mean(np.array(epoch_disc_loss_real), axis=0)
                discriminator_train_loss_fake = np.mean(np.array(epoch_disc_loss_fake), axis=0)
                ...
                train_history['discriminator_r'].append(discriminator_train_loss_real)
                train_history['discriminator_f'].append(discriminator_train_loss_fake)
                ...
                print(ROW_FMT.format('discriminator real (train)', *train_history['discriminator_r'][-1]))
                print(ROW_FMT.format('discriminator fake (train)', *train_history['discriminator_f'][-1]))
            ...
            if epoch % 1 == 0:
                ...
                for i in range(c):
                    ...
                    sampled_labels = np.array([num for _ in range(1) for num in range(1)])
